...
public class EnemyManager : MonoBehaviour
{
	...
	public int[] enemyLife = new int[20];
	public bool[] isDead = new bool[20];
	public bool[] hasKey;
	...
	void Update ()
	{
		...
		for (int i = 1; i < enemyGOs.Length; i++) {
			if (enemyLife [1] < 0 && enemyLife [2] < 0 && enemyLife [3] < 0 && enemyLife [16] < 0 && hasKey [0] == false) {
				...
				UIManager._instance.addKey ();
				hasKey [4] = true;
			} else if (enemyLife [15] < 0 && enemyLife [18] < 0 && enemyLife [17] < 0 && hasKey [5] == false) {
				Instantiate (keyPrefab, transform.position, Quaternion.Euler (transform.eulerAngles));
				UIManager._instance.addKey ();
				hasKey [5] = true;
			}
